...
//
//
pyr_context *io_close_file(babel_env *be, FILE *file){ // io_close_file#
...
mword *io_slurp8(babel_env *be, char *filename){ // io_slurp8#
    ...
    FILE *f = io_open_file(pc, (mword*)filename, "rb");
    mword file_size = io_file_size(pc, f);

    mword *file_buffer = mem_new_str(pc, file_size, ' ');
...
mword *io_slurp(babel_env *be, char *filename){ // io_slurp#
    ...
    mword *result = io_slurp8(pc, filename);
//    array_trunc(pc, result, size(result)-1);
    array_shrink(pc, result, 0, size(result)-1, MWORD_ASIZE);
...
void io_spit(babel_env *be, char *filename, mword *fileout, access_size asize, fileout_type ft){ // io_spit#
    ...
    if(ft == OVERWRITE){
        pFile = io_open_file(pc, (mword*)filename, "wb");
    }
    else{ // ft = APPEND
        pFile = io_open_file(pc, (mword*)filename, "ab");
    }
