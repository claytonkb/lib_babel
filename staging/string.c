...
mword *string_c2b(babel_env *be, char *string, mword max_safe_length){ // string_c2b#
    ...
    mword *result = mem_new_str(pc, char_length, ' ');
...
//    mword *result = mem_new_val(pc, u8_length+1, 0);
    mword *result = mem_new_str(pc, (u8_length*4), 0);

    mword length = u8_toucs((uint32_t *)result, u8_length+1, (char *)string, length8);

    array_shrink(pc, result, 0, (length*4), BYTE_ASIZE);
