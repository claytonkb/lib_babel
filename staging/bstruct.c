...
void bstruct_clean(babel_env *be, mword *bs){ // bstruct_clean#
    ...
    if( is_ptr(bs) ){
        ...
        for(i=0; i<num_elem; i++){
            bstruct_clean(pc, (mword *)*(bs+i));
        }
    }
    else if(is_tptr(bs)){
        bstruct_clean(pc, bs+HASH_SIZE+1);
    }
...
mword _fn_recurse(babel_env *be, mword *bs, bstruct_op_fn_ptr bfn, void *v){ // _fn_recurse#
    ...
    if( is_ptr(bs) ){
        ...
        mark_traversed_U(bs);
        for(i=0; i<num_elem; i++){
            if(!_fn_recurse(pc, (mword *)*(bs+i),bfn,v)){
                return 0;
            }
        }
        return 1;
        ...
        _fn_recurse(pc, (mword*)*tptr_ptr, bfn, v);
    }
    else{
        mark_traversed_U(bs);
    }
...
mword bstruct_mu(babel_env *be, mword *bs){ // _mu#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rmu, &counter);
    return counter;
...
mword bstruct_nar(babel_env *be, mword *bs){ // _nar#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rnar, &counter);
    return counter;
...
mword bstruct_nlf(babel_env *be, mword *bs){ // _nlf#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rnlf, &counter);
    return counter;
...
mword bstruct_nin(babel_env *be, mword *bs){ // _nin#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rnin, &counter);
    return counter;
...
mword bstruct_ntag(babel_env *be, mword *bs){ // _ntag#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rntag, &counter);
    return counter;
...
mword bstruct_nva(babel_env *be, mword *bs){ // _nva#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rnva, &counter);
    return counter;
...
mword bstruct_nptr(babel_env *be, mword *bs){ // _nptr#
    ...
    mword counter=0;
    bstruct_recurse(pc, bs, bstruct_rnptr, &counter);
    return counter;
...
mword *bstruct_cp(babel_env *be, mword *bs){ // bstruct_cp#
    ...
    mword *temp = bstruct_unload(pc, bs);

    bs = bstruct_load(pc, temp, size(temp));
...
mword *bstruct_unload(babel_env *be, mword *bs){
    ...
    if(is_val(bs)){
        dest = mem_new_val(pc, alloc_size(bs), 0);
        memcpy(dest,(bs-1),(size_t)UNITS_MTO8(size(dest)));
        return dest;
    }

    dest = mem_new_val(pc, bstruct_mu(pc, bs), 0);

    mword *span_array = bstruct_to_array(pc, bs);
    sort(pc, span_array, VAL);

    mword *offset_array = mem_new_val(pc, size(span_array), 0xff);

    mword dest_offset = 0;

    bstruct_unload_r(pc, bs, dest, &dest_offset, span_array, offset_array);
    bstruct_clean(pc, bs);
...
mword bstruct_load_str_r( // bstruct_load_str_r#
    ...
    if( is_traversed_U(bs) ){
        return offset_array[array_search(pc, span_array, (mword*)(&bs), VAL)];
    }
    ...
    *dest_offset             = *dest_offset+1;
    ...
    set_offset_for_ptr(pc, span_array, bs, offset_array, this_offset);
    ...
    if(is_tptr(bs)){ // is_tptr
        ...
        *dest_offset = *dest_offset + 1;
        ...
        ldv(dest,tptr_offset)
            = bstruct_unload_r(pc, (mword*)*tptr_ptr, dest, dest_offset, span_array, offset_array);
        ...
        *dest_offset = *dest_offset + num_elem;
        ...
        for(i=0; i<num_elem; i++){
            ldv(dest,local_offset+i) 
                = bstruct_unload_r(pc, rdp(bs,i), dest, dest_offset, span_array, offset_array);
        }
...
void set_offset_for_ptr(
    ...
    mword span_offset = array_search(pc, span_array, (mword*)(&ptr), VAL);
...
mword *bstruct_to_array(babel_env *be, mword *bs){  // bstruct_to_array#
    ...
    mword num_arrays = bstruct_nar(pc, bs);

    mword *arr_list = mem_new_ptr(pc, num_arrays);
    mword offset = 0;

    bstruct_to_array_r(pc, bs, arr_list, &offset);
    bstruct_clean(pc, bs);
...
void bstruct_to_array_r(babel_env *be, mword *bs, mword *arr_list, mword *offset){ // bstruct_to_array_r#
    ...
    *offset = *offset+1;
    ...
    if(is_tptr(bs)){
        ...
        bstruct_to_array_r(pc, tcar(bs), arr_list, offset);
        ...
        for(i=0; i<num_elem; i++){
            bstruct_to_array_r(pc, (mword*)rdp(bs,i), arr_list, offset);
        }
...
//
//    if(is_tptr(find_bs)){
//        result = tptr_find_tag_unsafe(pc, target_bs, find_bs);
//    }
//    else if(is_val(find_bs)){
//        result = array_find_val(pc, target_bs, find_bs);
//    }
//    else{ // is_ptr(find_bs)
//        result = array_find_ptr(pc, target_bs, find_bs);
//    }
//
//    return result;
//
