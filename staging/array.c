...
// XXX DEPRECATED XXX see access.*
//
arr array_read(babel_env *be, mword *bs, mword offset, access_size asize){ // array_read#
...
// XXX DEPRECATED XXX see access.*
//
arr array_write(babel_env *be, mword *bs, mword offset, mword *payload, access_size asize){ // array_write#
...
arr array_th(babel_env *be, mword *bs, mword entry){ // array_th#
    ...
    if(is_val(bs)){
        return _val(pc, rdv(bs, entry%size(bs)));
    }
...
//    mword strsize = size(string) - 1;
//    mword last_mword = rdv(string, strsize);
//    mword alignment = array1_dec_align(pc, last_mword);
//
//    if(last_mword){
...
//    char *result = (char*)mem_new_str(pc, arr_size, ' ');
//
//    int i;
//
...
//    mword arr8_size = array8_size(pc, array8);
//    mword *result = mem_new_val(pc, arr8_size, 0);
//
//    int i;
//
...
//
//    mword arr_size  = size(array);
//    mword *result   = _newbits(pc, arr_size);
//
//    int i;
//
//    for(i=0; i<arr_size; i++){
...
//
//    mword arr1_size  = array1_size(pc, array1);
//    mword *result = mem_new_val(pc, arr1_size, 0);
//
//    int i;
//
...
//        result = (char*)mem_new_valz(pc, UNITS_8TOM(s_left+s_right));
//    }
//    else if(is_ptr(right) && is_ptr(left)){
//        s_left  = sfield(left);
//        s_right = sfield(right);
//        result = (char*)mem_new_ptr(pc, UNITS_8TOM(s_left+s_right));
//    }
...
//    }
//
//    size_left  = array8_size(pc, left);
//    size_right = array8_size(pc, right);
//
//    result = (char*)mem_new_str(pc, size_left+size_right, '\0');
//
...
//    }
//
//    size_left  = array1_size(pc, left);
//    size_right = array1_size(pc, right);
//
//    if(   (size_left  % BITS_PER_BYTE == 0)
//       && (size_right % BITS_PER_BYTE == 0)) // array8_cat is *much* faster
//        return array8_cat(pc, left, right);
//
//    result = (char*)_newbits(pc, size_left+size_right);
//
//    // The hard part...
//
////    memcpy(result,           left,  size_left);
...
mword *array_slice(babel_env *be, mword *array, mword start, mword end){ // array_slice#
    ...
    if(end>start){
        ...
        if(is_val(array)){
            result = mem_new_val(pc, arr_size, 0);
        }
        else{
            result = mem_new_ptr(pc, arr_size);
        }

        array_move(pc, result, 0, array, start, arr_size, MWORD_ASIZE);
...
mword *array8_slice(babel_env *be, mword *array, mword start, mword end){ // array8_slice#
    ...
    mword *result = _val(pc, 0); // Empty-string
    ...
    if(end>start){
        ...
        result = mem_new_str(pc, end-start, ' ');
        array_move(pc, result, 0, array, start, end-start, BYTE_ASIZE);
...
//
//    array1_move_unsafe(pc, (dest+dest_begin_mword), dest_begin, (src+src_begin_mword), src_begin, size_arg);
//
//}
//
...
//        array1_move_split_0_0(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==1) && (num_src_splits==0)){
////_say("splits - dest:1 src:0");
//        array1_move_split_1_0(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==0) && (num_src_splits==1)){
////_say("splits - dest:0 src:1");
//        array1_move_split_0_1(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==1) && (num_src_splits==1)){
////_say("splits - dest:1 src:1");
//        array1_move_split_1_1(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==2) && (num_src_splits==1)){
////_say("splits - dest:2 src:1");
//        array1_move_split_2_1(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==1) && (num_src_splits==2)){
////_say("splits - dest:1 src:2");
//        array1_move_split_1_2(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else if((num_dest_splits==2) && (num_src_splits==2)){
////_say("splits - dest:n src:n");
//        array1_move_split_n(pc, dest, dest_begin, src, src_begin, size_arg);
//    }
//    else{
//        _pigs_fly;
//    }
...
//    array1_move_unsafe(pc, (dest+ctr), dest_begin, (src+ctr), src_begin, i);
//
//}
//
//
...
mword *array_to_string(babel_env *be, mword *array){ // array_to_string#
    ...
    result = mem_new_val(pc, arlength,0);
...
//
//                    if( !array_cmp_lex(pc, curr_ptr_elem, val, MWORD_ASIZE) ){
//
//                        // push onto val_list
//                        if(is_nil(val_list)){
//                            val_list = list_cons(pc, (curr_span_elem+j), nil);
//                        }
//                        else{
//                            list_unshift(pc, val_list, (curr_span_elem+j));
//                        }
//
//                    }
//
...
//
//                    if( !array_cmp_lex(pc, curr_ptr_elem, ptr, MWORD_ASIZE) ){
//
//                        // push onto ptr_list
//                        if(is_nil(ptr_list)){
//                            ptr_list = list_cons(pc, (curr_span_elem+j), nil);
//                        }
//                        else{
//                            list_unshift(pc, ptr_list, (curr_span_elem+j));
//                        }
//
//                    }
//
...
//            last_cons = list_cons(pc, rdp(arr,i),last_cons);
//        }
//    }
//    else{
//        for(i=size(arr)-1;i>=0;i--){
//            entry = mem_new_val(pc, 1, 0);
//            *entry = rdv(arr,i);
//            last_cons = list_cons(pc, entry,last_cons);
//        }
//    }
//
//    return last_cons;
...
mword array_search(babel_env *be, mword *array, mword *target, sort_type st){ // array_search#
    ...
    return array_search_binary(pc, array, (mword*)(array+size(array)), target, st);
    ...
    if(is_val(array) || (st == VAL)){
        ...
        ////////////////////////// aop-array ////////////////////////////////
            ...
            while(local_array_size){
                ...
                if(array_lt(pc, guess, target)){
                    lower_bound = guess_index;
                    guess_index += shift;
                }
                else if(array_gt(pc, guess, target)){
                    upper_bound = guess_index;
                    guess_index -= shift;
                }
                else if(array_eq(pc, guess, target)){
                    return guess_index;
                }
                else{
                    _pigs_fly;
        ...
        ////////////////////////// aop-array8 ///////////////////////////////
            ...
            while(local_array_size){
                ...
                if(array8_lt(pc, guess, target)){
                    lower_bound = guess_index;
                    guess_index += shift;
                }
                else if(array8_gt(pc, guess, target)){
                    upper_bound = guess_index;
                    guess_index -= shift;
                }
                else if(array8_eq(pc, guess, target)){
                    return guess_index;
                }
                else{
                    _pigs_fly;
...
mword array_search_linear(babel_env *be, mword *array, int start, int end, mword *target, sort_type st){ // array_search_linear#
    ...
    if(is_val(array) || st == VAL){
        ...
        for(; i<end; i++){
            if(array_eq(pc, key_aop(array,i), target)){
                return i;
            }
        }
    }
        ...
        for(; i<end; i++){
            if(array8_eq(pc, key_aop(array,i), target)){
                return i;
            }
        }
    }
...
//
//    if(all_vals){
//        result = (char*)mem_new_valz(pc, total_size);
//    }
//    else if(all_ptrs){
//        result = (char*)mem_new_ptr(pc, total_size);
//    }
//    else{
//        return (blob)nil;
