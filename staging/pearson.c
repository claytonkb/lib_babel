...
//
void pearson16(char *result, char *init, char *key, mword strlen){ // pearson16#
//_trace;
//printf("%s\n", key);
    int i;
    char temp;
...
void pearson_marsaglia16(char *result, char *init, char *key, mword strlen){ // pearson_marsaglia16#
    ...
    pearson_marsaglia_fuzz(1); // resets the PRNG...
    ...
    for(i=0;i<strlen;i++){
        ...
        for(j=0;j<HASH_BYTE_SIZE-1;j++){
            ...
            result[j] ^= (pearson_marsaglia_fuzz(0) & 0xff);
        }

        result[HASH_BYTE_SIZE-1] = pearson16_byte_perm[ (0 * PERM_TABLE_WIDTH) + ((temp ^ key[i]) & 0xff) ];
        result[HASH_BYTE_SIZE-1] ^= (pearson_marsaglia_fuzz(0) & 0xff);
